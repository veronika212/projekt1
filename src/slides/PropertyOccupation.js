...
const PropertyOccupation = ({
  ...
  const handleNextClick = () => {
    let slideNumber = 7;
    let currentStep = 3;
    if (selectedState === "Selbst bewohnt" || selectedState === "Leerstehend") {
      slideNumber = 6;
      currentStep = 4;
      if (propertyType === "Gewerbe") {
        slideNumber = 13;
    ...
    if (
      ...
      (propertyType === "Gewerbe" && selectedState === "Zum Teil vermietet")
    ) {
      slideNumber = 13;
      currentStep = 3.5;
    }
    ...
    <div>
      <div className="tiles-wrapper">
        ...
        <Tile
          ...
          selected={selectedState === "Leerstehend" ? true : false}
          className={error === true ? "tile-error" : null}
        />
        <Tile
          handleOnClick={() =>
        ...
        <Tile
          handleOnClick={() =>
            onTileClick(7, 3.5, 10, {
              key: "propertyOccupation",
              value: "Zum Teil vermietet"
            })
          }
          title={"Zum Teil vermietet"}
          iconName="icon icon--status-teilweise"
          selected={selectedState === "Zum Teil vermietet" ? true : false}
          className={error === true ? "tile-error" : null}
        />
      </div>
